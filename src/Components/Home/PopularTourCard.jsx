...
function PopularTourCard({ data }) {
    ...
    return (
        <div className='w-full'>
            <h3 className='text-center py-4 font-semibold md:text-2xl'>Explore By Fixed Departures
            </h3>
            <div className='h-full flex flex-wrap md:gap-6 justify-between'>
                {data.map((item, index) => (
                    <div key={index} onClick={() => navigate(`Tour/${item.link}`)} className='h-[45vh] sm:w-[100%] md:w-[48%] lg:w-[30%] rounded-xl cursor-pointer duration-300 shadow-md hover:shadow-md border hover:shadow-yellow-500'>
